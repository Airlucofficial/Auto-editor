...
if (
    ...
    and "window._SEEK_TO" in text
    and 'className:"tl__playhead",style:{left:O(i)},onPointerDown:w' in text
):
...
# 4e. Expose seek callback and time state to window
s_seek = "th=(0,l.useCallback)(e=>{let t=eE.current;if(!t)return;let a=Math.min(Math.max(e,0),i);t.currentTime=a,eB(a)},[i]);"
if s_seek in text:
    r_seek = (
        "th=(0,l.useCallback)(e=>{let t=eE.current;if(!t)return;let a=Math.min(Math.max(e,0),i);t.currentTime=a,eB(a)},[i]);"
        "if(typeof window!==\"undefined\"){"
        "window._SEEK_TO=th;"
        "window._GET_CURRENT_TIME=()=>eU;"
        "window._TIMELINE_TOTAL_DURATION=i;"
        "};"
    )
    text = text.replace(s_seek, r_seek, 1)

# 4f. Attach onPointerDown to playhead and playhead grip in component M
s_playhead = '(0,n.jsx)("div",{className:"tl__playhead",style:{left:O(i)},children:(0,n.jsx)("span",{className:"tl__playhead-grip"})})'
if s_playhead in text:
    r_playhead = '(0,n.jsx)("div",{className:"tl__playhead",style:{left:O(i)},onPointerDown:w,children:(0,n.jsx)("span",{className:"tl__playhead-grip",onPointerDown:w})})'
    text = text.replace(s_playhead, r_playhead, 1)
